...
```
- **중요: ③ 직후 반드시 ④를 곧바로 실행하세요.** ④(nblm_download_audio.js)는 오디오가 완성될 때까지 **스스로 폴링하며 블로킹 대기**(보통 8~15분, 최대 30분)한 뒤 파일을 저장하고 끝납니다. Bash timeout을 35분(2100000ms)으로 지정해 호출하면 됩니다. **"생성 중이니 완성되면 진행하겠다"는 식으로 ④·7단계를 남겨두고 응답을 끝내는 것을 금지합니다** — 헤드리스 실행에서는 이어서 재개할 사용자가 없어 그날 오디오·배포가 통째로 누락됩니다(2026-07-02 테스트에서 실제 발생).
